...
## File Enumeration
* [__init__.py](file:///home/castincar/hermes-agent/acp_adapter/__init__.py): Simple package initialization file identifying the directory as the ACP adapter module.
* [__main__.py](file:///home/castincar/hermes-agent/acp_adapter/__main__.py): Entry point wrapper allowing execution of the adapter via `python -m acp_adapter`.
* [auth.py](file:///home/castincar/hermes-agent/acp_adapter/auth.py): Resolves the active model provider configuration and exposes available authentication methods (agent-managed or terminal-based) to the ACP client during the handshake.
...
* [edit_approval.py](file:///home/castincar/hermes-agent/acp_adapter/edit_approval.py): Manages pre-execution approval for file modification tools (`write_file`, `patch`). It binds a requester callback to a `ContextVar` that prompts the client for interactive authorization.
...
* [entry.py](file:///home/castincar/hermes-agent/acp_adapter/entry.py): CLI entry point that processes CLI arguments, sets up logging to stderr (leaving stdout clean for JSON-RPC messages), ensures environment setup, and executes `acp.run_agent(...)`.
* [events.py](file:///home/castincar/hermes-agent/acp_adapter/events.py): Contains callback factories that map internal agent execution hooks to ACP client session updates.
...
* [permissions.py](file:///home/castincar/hermes-agent/acp_adapter/permissions.py): Bridges dangerous commands (e.g. arbitrary terminal execution) to the client using the ACP permission system.
...
* [provenance.py](file:///home/castincar/hermes-agent/acp_adapter/provenance.py): Builds session lineage metadata under `_meta.hermes` by traversing the parent-child history (including compression rotations) in the sqlite database.
  - `build_session_provenance()`: Resolves parent links, root session, and compression depth for the active session.
* [server.py](file:///home/castincar/hermes-agent/acp_adapter/server.py): The main server implementation (`HermesACPAgent`) inheriting from `acp.Agent`. It processes incoming JSON-RPC methods, routes prompt execution to background threads, manages configuration options, and intercepts local slash commands.
* [session.py](file:///home/castincar/hermes-agent/acp_adapter/session.py): Thread-safe session manager (`SessionManager`) mapping ACP sessions to `AIAgent` instances. Manages in-memory state and SQL-based persistence/restoration, and handles Windows-to-WSL workspace path translations.
  - `SessionManager`: Creates, restores, forks, lists, and cleans up active sessions.
* [tools.py](file:///home/castincar/hermes-agent/acp_adapter/tools.py): Maps Hermes tools (e.g., `read_file`, `terminal`, `web_search`) to ACP `ToolKind` values and constructs rich start/complete notification payloads (like diff blocks and execution summaries).
