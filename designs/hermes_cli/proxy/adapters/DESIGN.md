...
## File Enumeration
* [__init__.py](../../../../hermes_cli/proxy/adapters/__init__.py): Adapter registry. `ADAPTERS` dict maps the `--provider` CLI string (`"nous"`, `"xai"`) to its adapter class. `get_adapter(name)` instantiates one by name (case-insensitive, trimmed); raises `ValueError` listing available names if unknown.
* [base.py](../../../../hermes_cli/proxy/adapters/base.py): Defines the `UpstreamCredential` frozen dataclass and the abstract `UpstreamAdapter` base class.
